...
class CreatebillviewController extends GetxController {
  ...
  setDefaultDate(){
    dateController.text=  dateController.text=DateFormat("dd-MM-yyyy").format(DateTime.now());
  }
  ...
  Future<void> selectDate() async {
    ...
    if (picked != null && picked != selectedDate) {
        ...
        selectedDate = picked;
        dateController.text=DateFormat("dd-MM-yyyy").format(DateTime.parse(selectedDate.toString()));
    ...
    for (var item in billingItems) {
      if (item.itemName == chipValue) {
        if (item.quantity != 1) {
          ...
          billingItems.removeWhere((element) => element.itemName==item.itemName);
  ...
  getInvoiceNumber() async {
    ...
    var response= await APIServices.getMethodWithHeaderDio(invoice,token:authToken );
...
var date=DateFormat('yyyy-MM-dd HH:mm:ss').format(selectedDate??DateTime.now());
    ...
    var data={
        ...
        "warrenty_type":selectewarenty=='KM'?1:2,
        "warrenty_value":warentyvalueController.text,
